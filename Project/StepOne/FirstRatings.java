...
public class FirstRatings {
    public ArrayList<Movie> loadMovies(String filename){
        ...
        for(CSVRecord r : parser){
              ...
              ans.add(new Movie(id, title, year, genre, director, country, poster, minutes));
        }
    ...
    public void testLoadMovies(){
        String filename = "./StepOne/data/ratedmovies_short.csv";
        ArrayList<Movie> movies = loadMovies(filename);
        for(Movie m : movies){
            System.out.println(m.toString());
        }
        System.out.println("In ratedmovies_short.csv are " + movies.size());
        filename = "./StepOne/data/ratedmoviesfull.csv";
        movies = loadMovies(filename);
        System.out.println("In ratedmoviesfull.csv are " + movies.size());
        int comedyCount = 0;
        int durationCount = 0;
        for(Movie m : movies) {
            if(m.getGenres().contains("Comedy")) comedyCount += 1;
            if(m.getMinutes()>150) durationCount += 1;
        }
        System.out.println("In ratedmoviesfull.csv are " + comedyCount + " comedy movies");
        System.out.println("In ratedmoviesfull.csv are " + durationCount + " >150 minutes movies");
    ...
    public ArrayList<Rater> loadRaters (String filename) {
        ...
        FileResource fr = new FileResource("./StepOne/data/" + filename + ".csv");
        CSVParser parser = fr.getCSVParser();
